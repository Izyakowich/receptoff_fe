...
const ProductsPage: React.FC = () => {
    ...
    const user = useUser();
    const isUserAuth = useIsAuth();
    const [generatingProducts, setGeneratingProducts] = useState<number[]>([]);
    ...
    const handleGenerateImage = async (productId: number) => {
        if (!isUserAuth) {
            toast.info('Авторизуйтесь для генерации изображений');
            return;
        }
        
        setGeneratingProducts(prev => [...prev, productId]);
        try {
            const response = await axios.post(
                `http://localhost:8000/products/${productId}/generateImage/`,
                {},
                { withCredentials: true }
            );
            ...
            if (response.data.image_url) {
                const updatedProducts = products.map(p => 
                    p.id === productId ? { ...p, src: response.data.image_url } : p
                );
                ...
                <div className={styles["main__page-cards"]}>
                    {products.map((product: product) => (
                        <OneCard 
                            ...
                            onGenerateClick={() => handleGenerateImage(product.id)}
                            isGenerating={generatingProducts.includes(product.id)}
                            title={product.title} 
                            price={Number(product.price)}
                            canGenerate={isUserAuth}
                        />
                    ))}
                </div>
